...
############################# getting data ###################################
Data="NGC7331-S001-R001-C001-r.fts"
D="Dark-S001-R003-C003-B2.fts"
F="combflatr.fits"
...
centrox = np.loadtxt('Xcentroid.txt')
centroy = np.loadtxt('Ycentroid.txt')
...
for i in range(len(catalogx)):
    B[i][0] = (f*catalogx[i]/p)
    B[i][1] = (f*catalogy[i]/p)
    B[i][2] = 1
    a[i][0] = detectedx[i]
    b[i][0] = detectedy[i]
...
T = np.matrix([[(F*c[0]), (F*c[1]),(c[2])], [(F*d[0]), (F*d[1]),(d[2])], [0,0,1]])
T2 = np.matrix([[(c[0]), (c[1]),(c[2])], [(d[0]), (d[1]),(d[2])], [0,0,1]])
...
#~~~~~~~~~~~~~~~~
plt.figure()
plt.plot (catalogx, catalogy, 'g.')
plt.plot (detectedx, detectedy, 'y+')
'''
#~~~~~~~~~~~~~~~~
plt.figure()
...
#plt.figure()
#plt.plot(detectedx,dx,'+')
#plt.plot(detectedy,dy,'.')
#plt.scatter(X2[w],Y2[w])
#plt.scatter(X,Y, 'g')
